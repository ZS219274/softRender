...
void Raster::interpolantLine(const Point& v0, const Point& v1, Point& target) {
  ...
  RGBA result;
  result.mR =
      static_cast<byte>(static_cast<float>(v1.color.mR) * weight +
                        (1.0f - weight) * static_cast<float>(v0.color.mR));
  result.mG =
      static_cast<byte>(static_cast<float>(v1.color.mG) * weight +
                        (1.0f - weight) * static_cast<float>(v0.color.mG));
  result.mB =
      static_cast<byte>(static_cast<float>(v1.color.mB) * weight +
                        (1.0f - weight) * static_cast<float>(v0.color.mB));
  result.mA =
      static_cast<byte>(static_cast<float>(v1.color.mA) * weight +
                        (1.0f - weight) * static_cast<float>(v0.color.mA));
...
void Raster::rasterizeTriangle(std::vector<Point>& results, const Point& v0,
  ...
  for (int i = minX; i <= maxX; ++i) {
    for (int j = minY; j <= maxY; ++j) {
      ...
      // Use cross product to determine position relation of point (i,j) to triangle edges
...
void Raster::interpolantTriangle(const Point& v0, const Point& v1,
  ...
  // Calculate total triangle area (using cross product, equals twice the area of parallelogram)
  ...
  // Calculate barycentric coordinate weights, ratios of sub-triangle areas to total area
  float weight0 = v0Area / sumArea;  
  float weight1 = v1Area / sumArea;  
  float weight2 = v2Area / sumArea;  

  RGBA result;
  // Get colors of three vertices
  auto c0 = v0.color;
  auto c1 = v1.color;
  auto c2 = v2.color;  
