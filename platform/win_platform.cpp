...
LRESULT CALLBACK WinPlatformWindow::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    WinPlatformWindow* window = reinterpret_cast<WinPlatformWindow*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
...
bool WinPlatformWindow::initialize(uint32_t width, uint32_t height) {
    ...
    mhBmp = CreateDIBSection(mCanvasDC, &bmpInfo, DIB_RGB_COLORS, &mCanvasBuffer, 0, 0);
...
void* WinPlatformWindow::getNativeHandle() {
    return mHwnd;
}